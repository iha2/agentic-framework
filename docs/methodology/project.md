...
### Authoritative narrative elsewhere (e.g. SharePoint)

Some organizations keep the **portfolio-facing** or leadership-approved project narrative in **SharePoint** (Word, PowerPoint, PDF, HTML). In that case, treat `project.md` in Git as the **agent-reference** summary: structured, current enough for implementation work, and linked from `AGENTS.md` to the authoritative artifact. Record which copy governs **approval** and who refreshes the repo file when SharePoint changes.
