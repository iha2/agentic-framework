...
Formal architecture decks, diagrams, and sign-off packages may live in **SharePoint** (Office, PDF, HTML exports). Markdown under `architecture/` in the repo should remain the **agent-loadable** technical reference. Declare in `AGENTS.md` which surface is authoritative for governance and how the markdown stays aligned with SharePoint.
