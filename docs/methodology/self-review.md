...
Treat graph output as advisory. Stock risk scores, test-gap counts, affected-flow counts, and token-savings percentages are signals to investigate, not approval criteria. See [Code review graph](../code-review-graph.md) for the methodology guidance and the planned tooling-specific fork direction.
