...
Pull requests package the completed deliverable for automated review, human review, and merge. The methodology uses draft-first PRs by default so implementation, evidence, and self-review can converge before the team asks for human approval.
...
| Companion board/tracker integration (repo-configured) | Supplies board-backed deliverable and task context when the repo uses a project board or external tracker for work items. |
