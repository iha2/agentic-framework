...
and test runners are deterministic enough to make review safer and cheaper. Treat these tools as part of the
delivery quality bar, not as optional polish.
...
- **Agent-context text hygiene**: deterministic scans for **invisible Unicode** and related **smuggling codepoints** in
  markdown, skill files, prompts, YAML/JSON, CSV, and source comments—material that agents often load as context. This
  reduces **invisible prompt injection** risk (hidden instructions humans do not see on review). **Supply chain** in skills, rules, MCP, hooks, and dependencies is a separate human playbook under `docs/security/`. One canonical write-up for Unicode smuggling **and** why deterministic linting matters: [Invisible text prompt injection, linting benefits, and deterministic checks](../code-quality/invisible-text-prompt-injection-and-linting.md); governance for trusted-looking config changes: [Human guidelines: supply chain risk in agent instructions](../security/agent-instruction-supply-chain-human-guidelines.md).
  This repository ships a concrete Unicode-scanning starting point under `tools/invisible_unicode_lint/`
  (`patterns.default.json` plus a small scanner; wire it in pre-commit/CI like other linters).
...
This methodology treats conformance as recurring maintenance:
...
- Repos that **give agents long-lived text** (markdown specs, `.agents/skills/`, config, comments) should add a
  **deterministic invisible-Unicode / smuggling gate** alongside markdown and YAML checks. ESLint’s
  `no-irregular-whitespace` helps JavaScript/TypeScript sources but does not replace a file-wide scanner driven by an
  explicit codepoint list; see the methodology’s `tools/invisible_unicode_lint/` and [Invisible text prompt injection, linting benefits, and deterministic checks](../code-quality/invisible-text-prompt-injection-and-linting.md).
...
- `python3 tools/invisible_unicode_lint/lint_invisible_unicode.py .` — invisible Unicode / smuggling gate (see [code quality doc](../code-quality/invisible-text-prompt-injection-and-linting.md); when vendored from methodology)
```
...
- [Invisible text prompt injection, linting benefits, and deterministic checks](../code-quality/invisible-text-prompt-injection-and-linting.md)
- [Human guidelines: supply chain risk in agent instructions](../security/agent-instruction-supply-chain-human-guidelines.md)
