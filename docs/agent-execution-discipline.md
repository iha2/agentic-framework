...
This document is the default operating guide for **agents** executing, verifying,
reviewing, or packaging work inside a repository under the **Agentic Software Delivery Methodology**. That methodology assumes **agents are the primary executors** of the delivery chain (implementation, tests, PRs, preparatory review) while **humans govern** specifications, standards, and merge approval. This file is intentionally generic. Repo
`AGENTS.md` files should link here, then declare only repo-specific overrides such as
integration branch, command wrappers, tracking system, execution-record location, and any **human-only** gates beyond the defaults described in methodology docs.
...
- External tracker comment, attachment, or work-item link (when declared in `AGENTS.md`)
