...
This document defines the required worktree structure for repos using the Agentic Engineering Framework methodology.
...
- execution artifacts can stay local, be committed intentionally, or be mirrored/tracked in an external system such as a declared project board or tracker
...
- `feature/rbac` is wrong for methodology sibling worktrees
- `docs/reorg-mw1` is wrong for methodology sibling worktrees
...
Git allows `/` in branch names, but that creates awkward nested directory paths if the same string is used for a sibling worktree directory. This methodology avoids that mismatch by standardizing on flat branch names when using sibling worktrees.
...
- the repo's `AGENTS.md` should declare the docs root, commonly `docs/` or another repo-specific root (some repos still use a legacy `.zazz/` layout)
...
- some repos also mirror, track, or store execution artifacts in an external system such as a declared project board or tracker
...
This document is the operational companion to [../methodology.md](../methodology.md).
