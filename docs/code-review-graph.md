...
This guide explains how teams using the Agentic Engineering Framework should use
...
For methodology adopters, treat it as advisory review context. It can help identify blast radius,
...
## Policy (this methodology)

Use `code-review-graph` as a utility behind the existing `pr-review` skill.
...
Default behavior in this methodology:
...
This keeps the bundled `pr-review` skill directory small and avoids competing review workflows. When an
...
## Planned fork (tooling)

The planned methodology-specific direction is a fork or wrapper that keeps the graph engine and context
extraction, but replaces or overlays the scoring and reviewability model.

The fork should optimize for methodology review questions:
...
Prefer explainable scoring over a single opaque number. A useful methodology score should
combine:
...
A methodology-oriented fork should support configurable synthetic edges for:
...
Recommended default for methodology PR review:
...
`pr-review` skill to use CLI output and optional MCP follow-up.
...
part of default methodology PR-review setup.
...
Full integration is opt-in and usually not needed for methodology repos:
...
proactive development-environment feature rather than an optional graph review utility.

## CLI And MCP

`code-review-graph` provides both a CLI and MCP tools.

For `pr-review`, prefer the CLI for the first graph pass:
...
For normal methodology PR review, prefer the PyPI package through `uvx`.
