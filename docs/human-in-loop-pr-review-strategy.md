...
The recommended methodology strategy is:
...
- a merged, approved deliverable specification for feature, architecture, refactor, or
  multi-layer work. In normal methodology workflow, approval means the specification PR was
...
the review axes read source files. This methodology treats `code-review-graph` as advisory context for
...
[Code review graph](code-review-graph.md) for current usage guidance and the
planned tooling-specific fork direction.
...
3. The builder or agent creates one isolated worktree lane for the stack, then creates one
   branch per stack slice using flat branch names that are safe for the methodology sibling-worktree
   layout.
