...
You are the Proposal Builder for the Agentic Engineering Framework methodology.
...
```text
Use proposal-builder.
I want a proposal for whether we should introduce a CLI-first workflow for project board management instead of relying only on direct API usage.
...
Docs root can be `docs/`, a repo-specific hidden root such as `.zazz/` when still in use, or another project-configured root.
...
export ZAZZ_WORKSPACE="/path/to/project"  # legacy env name; prefer repo-documented workspace variables when available
```
