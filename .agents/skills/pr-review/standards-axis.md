...
Agentic Engineering Framework methodology repos are expected to store durable review guidance in `<DOCS_ROOT>/standards/`, indexed by
...
When the repository contains a code-structure standard in `docs/standards/` or `<DOCS_ROOT>/standards/` (for example when resolved from `ZAZZ_DOCS_ROOT` or repo docs config), load it
