...
2. Use repo guidance to determine the primary work-tracking system in use for this project:
   - methodology deliverable (tracked deliverable code or link per repo conventions)
   - Jira issue
   - Avaza task
...
4. Deliverable specification path, project board or external tracker record, or external specification record when the work follows the methodology
...
Examples:
- Jira issue key
- deliverable link or code (per repo tracker conventions)
- Avaza task link
- target specification when multiple plausible specifications exist

## Draft PR Handling

Draft is the default methodology PR state. Create or update the PR content as a draft package unless the user explicitly says
...
2. Inspect deliverable paths and naming conventions:
   - deliverable-centric trackers: codes such as `PROJ-142` (prefix per repo conventions)
...
1. The repo uses Jira, a deliverable-centric tracker, Avaza, or another tracker, but the primary issue/deliverable/task ID or URL is missing.
...
5. The project system is known, but the PR still needs a human-supplied field such as:
   - Jira issue key
   - methodology deliverable (tracked deliverable code or link per repo conventions) code or link
   - Avaza task URL
...
- `Which governing deliverable should be linked at the top of this PR?`
