...
- **Run log**:
  ...
  tracker records are also valid when declared. Repos that do not use a project board or external tracker may rely
  exclusively on `<DOCS_ROOT>/execution/`. When the Owner uses a project board or external tracker, use it as the
...
- **Externally stored specification**:
  a project board or external tracker, or the repo-declared tracking system, with a stable identifier linked from
