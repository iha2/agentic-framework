...
This reference is the portable spec-builder summary of the Agentic Engineering Framework methodology. Repo-local methodology documents and
...
Specifications may also live in a project board, external tracker, or another declared external issue system when the repo policy says they are not
...
file, committed support artifact, project board or external tracker note, external tracker entry, or a combination.
Repos that do not use a project board or external tracker may use `<DOCS_ROOT>/execution/` as the exclusive execution-record surface.
When a project board or external tracker is the declared surface, it acts as the centralized execution record for run logs, handoff notes, QA
