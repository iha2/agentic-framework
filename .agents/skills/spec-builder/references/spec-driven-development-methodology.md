...
This reference is the portable spec-builder summary of the Agentic Software Delivery Methodology. Repo-local methodology documents and
...
Specifications may also live in project board or external tracker or another declared external tracker when the repo policy says they are not
...
Repos that do not use project board or external tracker may use `<DOCS_ROOT>/execution/` as the exclusive execution-record surface.
When project board or external tracker is the declared surface, it acts as the centralized execution record for run logs, handoff notes, QA
