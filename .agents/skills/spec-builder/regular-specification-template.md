...
Repos that do not use a project board or external tracker may rely exclusively on `<DOCS_ROOT>/execution/` for
run logs, handoff notes, QA findings, and related execution records.

When the Owner uses a project board or external tracker, the run log, handoff notes, QA findings, and related
