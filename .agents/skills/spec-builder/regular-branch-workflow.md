...
- The operating model determines whether `specifications/` is tracked, ignored, mirrored
  to a project board, Jira, or another declared tracker, or promoted elsewhere after implementation.
...
- External specification storage: project board or external tracker, Jira, or another
...
- Run log path/location: follow the repo's declared policy; use an ignored local file,
  ...
  Repos that do not use project board or external tracker may rely exclusively on `<DOCS_ROOT>/execution/`.
  When the Owner uses project board or external tracker, prefer it for execution records that need to be shared
