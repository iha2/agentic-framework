...
- External specification storage: a project board or external tracker, or the repo-declared tracking system when
  specifications are not committed in Git.
- Run log path/location: follow the repo's declared policy; use an ignored local file,
  committed support artifact, project board or external tracker note, external tracker entry, or combination.
  Repos that do not use a project board or external tracker may rely exclusively on `<DOCS_ROOT>/execution/`.
  When the Owner uses a project board or external tracker, prefer it for execution records that need to be shared
...
4. Resolve the specification path under `<DOCS_ROOT>/specifications/`, or the project board or external tracker location when the repo does not commit specifications.
