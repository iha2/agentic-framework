...
repo-local orientation document as the base for methodology artifacts. Example
...
repos may use a non-`docs/` root declared in `AGENTS.md` (for example `.agentic/`). Feature documents belong under `<DOCS_ROOT>/features/`, not under imported reference-guide subtrees.
...
1. **A timeline note.** Use a related project milestone, target window, or `TBD`. Do not invent a project milestone
   date in the feature document when a project planning surface such as Jira, GitHub Projects, or another declared tracker owns project milestone scheduling.
